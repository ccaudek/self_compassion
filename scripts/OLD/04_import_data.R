# Emergency psychology -- Toscana and Lombardia
...
df_toscana   <- read_excel(here("data", "raw", "toscana.xlsx"))
df_lombardia <- read_excel(here("data", "raw", "lombardia.xlsx"))

df <- bind_rows(df_toscana, df_lombardia)
...
url <- "https://docs.google.com/spreadsheets/d/1UzQSB9yHiag9NvNQmvvB9B2NU8rfb4bR5aLwNcek9Q8/edit#gid=1062408954"
thedat <- read_sheet(url)
...
# Strategie di evitamento (Negazione + Umorismo + Uso di droga + Distacco comportamentale + 
# Distacco mentale), 
# Attitudine positiva (Accettazione + Contenimento + Reinterpretazione positiva), 
# Orientamento al problema (Soppressione + Pianifi cazione + Attività) 
# Religione (Sica, Novara, Dorz e Sanavio, 1997a)
# 
...
# value by using the following formula: abs(x - max).
...
# total score
cope_nvi <- social_support + avoiding_strategies + positive_attitude +
  problem_orientation + transcendent_orientation

# avoinding strategies Reversed:
avoiding_strategies_R <- 
  abs(cope_nvi$ii6 - 5) + abs(cope_nvi$ii27 - 5) + abs(cope_nvi$ii40 - 5) + 
  abs(cope_nvi$ii57 - 5) + abs(cope_nvi$ii9 - 5) + abs(cope_nvi$ii24 - 5) + 
  abs(cope_nvi$ii37 - 5) + abs(cope_nvi$ii51 - 5) + abs(cope_nvi$ii2 - 5) + 
  abs(cope_nvi$ii16 - 5) + abs(cope_nvi$ii31 - 5) + abs(cope_nvi$ii43 - 5) +  
  abs(cope_nvi$ii12 - 5) + abs(cope_nvi$ii26 - 5) + abs(cope_nvi$ii43 - 5) +
  abs(cope_nvi$ii35 - 5) + abs(cope_nvi$ii53 - 5)

# trascendent orientation Reversed
transcendent_orientation_R <- cope_nvi$ii8 + cope_nvi$ii20 + 
  cope_nvi$ii36 + cope_nvi$ii50 + abs(cope_nvi$ii7 - 5) + 
  abs(cope_nvi$ii18 - 5) + abs(cope_nvi$ii48 - 5) + abs(cope_nvi$ii60 - 5)


# Post Traumatic Growth Inventory (PTGI) ----------------------------------


# PTGI is an instrument for assessing positive outcomes reported by persons who 
...
# New Possibilities
new_possibilities <- ptgi$iii11 + ptgi$iii7 + ptgi$iii3 + ptgi$iii17 + ptgi$iii14
# hist(new_possibilities)

# Personal Strength
personal_strength <- ptgi$iii10 + ptgi$iii19 + ptgi$iii4 + ptgi$iii12
...
# The Self-Compassion Scale (SCS) is used to assess how people treated themselves 
# in difficult times (Neff 2003a). 
...
# - Mindfulness, 
# - Overidentification
# I punteggi delle sottoscale del scs sono ottenuti calcolando la media delle risposte 
# nei rispettivi item.
# Quindi calcolare la media dei punteggi medi delle sei sottoscale. COME SI FA?
# In this approach any mean scale score ranging from 1 to 2.9 could be considered low 
# support; a score of 3 to 5 could be considered moderate support; a score from 5.1 
# to 7 could be considered high support. 
# This approach would seem to have more validity, but if you have very few respondents 
# in any of the groups, it could be problematic.
...
# Self-Judgment
self_judgment <- 
  abs(scs$iiiii1 - 6) + abs(scs$iiiii8 - 6) + abs(scs$iiiii11 - 6) + 
  abs(scs$iiiii16 - 6) + abs(scs$iiiii21 - 6)
# hist(self_judgment)
...
# The MSPSS is a 12-item scale designed to measure perceived social support 
# from three sources:
# - Family, 
# - Friends, 
# - a Significant other
...
saveRDS(thedat, file = here("data", "rescue_workers.Rds"))
